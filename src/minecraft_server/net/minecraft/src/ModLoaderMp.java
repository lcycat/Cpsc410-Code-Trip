...
public class ModLoaderMp
{
    ...
    private static void sendPacketToAll(Packet packet)
    {
        if (packet != null)
        {
            for (int i = 0; i < ModLoader.getMinecraftServerInstance().serverConfigManager.playerEntities.size(); i++)
            {
                ((EntityPlayerMP)ModLoader.getMinecraftServerInstance().serverConfigManager.playerEntities.get(i)).playerNetServerHandler.sendPacket(packet);
            }
        }
    }
    ...
    {
        List list = ModLoader.getMinecraftServerInstance().serverConfigManager.playerEntities;
    ...
    public static void sendChatToOps(String s)
    {
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            if (ModLoader.getMinecraftServerInstance().serverConfigManager.isOp(entityplayermp.username))
            {
                entityplayermp.playerNetServerHandler.sendPacket(new Packet3Chat(s));
            }
        }
