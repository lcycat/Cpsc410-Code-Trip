...
class PlayerInstance
{
    ...
    public void addPlayer(EntityPlayerMP par1EntityPlayerMP)
    {
        ...
        {
            throw new IllegalStateException((new StringBuilder()).append("Failed to add player. ").append(par1EntityPlayerMP).append(" already is in chunk ").append(currentChunk.chunkXPos).append(", ").append(currentChunk.chunkZPos).toString());
        }
        else
        {
    ...
    public void removePlayer(EntityPlayerMP par1EntityPlayerMP)
    {
        ...
        par1EntityPlayerMP.playerNetServerHandler.sendPacket(new Packet51MapChunk(PlayerManager.func_72692_a(playerManager).getChunkFromChunkCoords(currentChunk.chunkXPos, currentChunk.chunkZPos), true, 0));
        players.remove(par1EntityPlayerMP);
        par1EntityPlayerMP.loadedChunks.remove(currentChunk);

        if (players.isEmpty())
        {
            long l = (long)currentChunk.chunkXPos + 0x7fffffffL | (long)currentChunk.chunkZPos + 0x7fffffffL << 32;
            PlayerManager.func_72689_b(playerManager).remove(l);

            if (numBlocksToUpdate > 0)
            {
                PlayerManager.func_72682_c(playerManager).remove(this);
            }

            playerManager.getMinecraftServer().theChunkProviderServer.dropChunk(currentChunk.chunkXPos, currentChunk.chunkZPos);
        }
    }
    ...
    public void markBlockNeedsUpdate(int par1, int par2, int par3)
    {
        if (numBlocksToUpdate == 0)
        {
            PlayerManager.func_72682_c(playerManager).add(this);
        }
    ...
    public void onUpdate()
    {
        ...
        if (numBlocksToUpdate == 1)
        {
            ...
            int k1 = currentChunk.chunkZPos * 16 + (blocksToUpdate[0] >> 8 & 0xf);
            sendPacketToPlayersInInstance(new Packet53BlockChange(i, l, k1, PlayerManager.func_72692_a(playerManager)));

            if (PlayerManager.func_72692_a(playerManager).blockHasTileEntity(i, l, k1))
            {
                updateTileEntity(PlayerManager.func_72692_a(playerManager).getBlockTileEntity(i, l, k1));
            }
        }
        else if (numBlocksToUpdate == 64)
        {
            int j = currentChunk.chunkXPos * 16;
            int i1 = currentChunk.chunkZPos * 16;
            sendPacketToPlayersInInstance(new Packet51MapChunk(PlayerManager.func_72692_a(playerManager).getChunkFromChunkCoords(currentChunk.chunkXPos, currentChunk.chunkZPos), false, field_73260_f));
            ...
            for (int l1 = 0; l1 < 16; l1++)
            {
                ...
                {
                    int j2 = l1 << 4;
                    List list = PlayerManager.func_72692_a(playerManager).getTileEntityList(j, j2, i1, j + 16, j2 + 16, i1 + 16);
        ...
        {
            sendPacketToPlayersInInstance(new Packet52MultiBlockChange(currentChunk.chunkXPos, currentChunk.chunkZPos, blocksToUpdate, numBlocksToUpdate, PlayerManager.func_72692_a(playerManager)));

            for (int k = 0; k < numBlocksToUpdate; k++)
            {
                int j1 = currentChunk.chunkXPos * 16 + (blocksToUpdate[k] >> 12 & 0xf);
                int i2 = blocksToUpdate[k] & 0xff;
                int k2 = currentChunk.chunkZPos * 16 + (blocksToUpdate[k] >> 8 & 0xf);

                if (PlayerManager.func_72692_a(playerManager).blockHasTileEntity(j1, i2, k2))
                {
                    updateTileEntity(PlayerManager.func_72692_a(playerManager).getBlockTileEntity(j1, i2, k2));
                }
            }
        }
