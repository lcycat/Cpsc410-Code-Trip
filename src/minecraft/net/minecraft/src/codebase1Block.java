...
public class codebase1Block extends Block {
	 public static CodeBase codebase1 = new CodeBase();
	...
	protected codebase1Block(int id, int textureIndex, Material mat) {
		 super(id, textureIndex, mat);
         super.setHardness(3.0f);
         super.setBlockName("codebase1");
    ...
    {   
		...
		generateCodeBase(par5EntityPlayer);
		...
		List<Town> towns = codebase1.getTown();
		...
		for(int a =0; a <codeSize; a++){
			...
			for(int b =0; b <codeSize; b++){
				if(townCount<townNum){
					...
					codebase1.getTown().get(townCount).setStationX(townX-blockdistance-3);
					codebase1.getTown().get(townCount).setStationY(townY-blockdistance-3);
					this.buildTown(world, townX, j, townY, t,townSize);
					//testMessage
					par5EntityPlayer.addChatMessage("Build a town:" +t.getName());
        ...
        int z = k - blockdistance;
		    		for(int b = 0;b < townSize*blocksize +1;b++)
		    	    {
		    			  int y = world.getTopSolidOrLiquidBlock(x, z);
				    		...
				    		world.setBlockWithNotify(x, y, z, townFence);
				    		world.setBlockWithNotify(x, y+1, z, townFence);
				    		world.setBlockWithNotify(x, y+2, z, townFence);
				    ...
				    z=k-size;
				    		for(int b = 0;b <2* size;b++)
				    	    {
				    			 ...
				    			 for(int n = 0; n <func; n++){
						    	    ...
						    	    {
						    		world.setBlockWithNotify(x, y, z, buildingblock);
						    	    y++; 
						    	    top = y;
						    	    }
	        ...
	        z = k - distanceBTW+1;
			    		for(int b = 0;b < blockdistance*2;b++)
			    	    {
			    			   y = world.getTopSolidOrLiquidBlock(x, z);
					    		...
					    		world.setBlockWithNotify(x, y, z, buildingFence);
		    ...
		    z = k + distanceBTW-1;
	    	 ...
	    	 //randomly distribute lava/diamond on buildings to represent bugs
		       for(int f = 0 ; f < bugs ; f++){
		    	   ...
		    	   world.setBlockWithNotify(x, random.nextInt(height-1), z, Block.lavaStill.blockID);
		       }
...
public void buildPlatform(World world, int i, int j, int k){
	 ...
	 int y = j-1;
        ...
        z = k;
   	    y = j-1;
	   	 for(int c = 0;c <2* (platformsize-1);c++)
		    {			
...
public void buildRail(World world,int z){
	...
	for(int i = 0; i < towns.size(); i ++){
		...
		for(int j = 0; j < linkedtowns.size();j++){
            ...
            while((x-x1)!=0){
	    		world.setBlockWithNotify(x, z, y, Block.railPowered.blockID);
	    		if((x-x1)<0){
	    			x++;
	    		}
	    		else{
...
public static void generateSampleCodeBase(){
	...
	buildE.setBugs(10);
	t1.addBuilding(buildA);
	t1.addBuilding(buildB);
	t1.addBuilding(buildE);
	t1.addBuilding(buildD);
	t2.addBuilding(buildC);
	t2.addBuilding(buildD);
	t3.addBuilding(buildE);
	t4.addBuilding(buildC);
	t4.addBuilding(buildD);
	t4.addBuilding(buildE);
	t5.addBuilding(buildC);
	t6.addBuilding(buildE);
	t5.addBuilding(buildE);
	t6.addBuilding(buildC);
	t6.addBuilding(buildA);
	t6.addBuilding(buildB);
	t1.addLinkedTown(t2);
	t1.addLinkedTown(t3);
	t2.addLinkedTown(t3);
	t4.addLinkedTown(t6);
	...
	t4.setName("HungryCat");
	t5.setName("SleepyDog");
	t6.setName("MadFish");
	codebase1.addTown(t1);
	codebase1.addTown(t2);
	codebase1.addTown(t3);
	codebase1.addTown(t4);
	codebase1.addTown(t5);
	codebase1.addTown(t6);
	codebase1.setName("Whatever");
}
...
public void generateCodeBase(EntityPlayer par5EntityPlayer){
	 String currDir = System.getProperty("user.dir");
		...
		// Temp value
		String pathCodeBase = currDir + "/CodeBases/CodeBase1";
		...
		codebase1 = xp1.runParsing(pathNCSSxml, pathFindBugsXML, pathJDepxml);
		//test info
		par5EntityPlayer.addChatMessage("codebase info:" +codebase1.name);
		for(int i = 0; i<codebase1.Towns.size();i++){
		String townName = codebase1.Towns.get(i).name;
