...
public class TcpConnection implements NetworkManager
{
            ...
            if (field_74468_e == 0 || System.currentTimeMillis() - ((Packet)dataPackets.get(0)).creationTimeMillis >= (long)field_74468_e)
            {
                ...
                if (packet != null)
                {
                    ...
                    if ((packet instanceof Packet252SharedKey) && !isOutputEncrypted)
                    {
                        ...
                        {
                            sharedKeyForEncryption = ((Packet252SharedKey)packet).func_73304_d();
                        }
    ...
    private boolean readPacket()
    {
        ...
        try
        {
            ...
            if (packet != null)
            {
                if ((packet instanceof Packet252SharedKey) && !isInputBeingDecrypted)
                {
                    if (theNetHandler.isServerHandler())
                    {
                        sharedKeyForEncryption = ((Packet252SharedKey)packet).func_73303_a(field_74463_A);
                    }
                ...
                if (!isServerTerminating)
                {
                    if (packet.isWritePacket() && theNetHandler.canProcessPackets())
                    {
                        field_74490_x = 0;
    ...
    private void decryptInputStream() throws IOException
    {
        ...
        socketInputStream = new DataInputStream(CryptManager.decryptInputStream(sharedKeyForEncryption, networkSocket.getInputStream()));
    }
    ...
    private void encryptOuputStream() throws IOException
    {
        ...
        socketOutputStream = new DataOutputStream(new BufferedOutputStream(CryptManager.encryptOuputStream(sharedKeyForEncryption, networkSocket.getOutputStream()), 5120));
    }
