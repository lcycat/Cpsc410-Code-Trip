...
public class ModLoaderMp
{
    ...
    public static final String VERSION = "1.3.1v1";
    ...
    public static void handleEntityTrackers(EntityTracker entitytracker, Entity entity)
    {
        ...
        for(Iterator iterator = entityTrackerMap.entrySet().iterator(); iterator.hasNext();)
        {
            ...
            {
                entitytracker.func_72785_a(entity, ((Integer)((Pair)entry.getValue()).getLeft()).intValue(), ((Integer)((Pair)entry.getValue()).getRight()).intValue(), true);
                return;
            }
        }
    ...
    private static void sendPacketToAll(Packet packet)
    {
        if(packet != null)
        {
            for(int i = 0; i < ModLoader.getMinecraftServerInstance().field_71318_t.field_72404_b.size(); i++)
            {
                ((EntityPlayerMP)ModLoader.getMinecraftServerInstance().field_71318_t.field_72404_b.get(i)).field_71135_a.func_72567_b(packet);
            }
    ...
    {
        MinecraftServer.field_71306_a.info(s);
    ...
    {
        WorldServer aworldserver[] = ModLoader.getMinecraftServerInstance().field_71305_c;
        for(int i = 0; i < aworldserver.length; i++)
        {
            if(((World) (aworldserver[i])).field_73010_i.contains(entityplayer))
            {
    ...
    private static void initializePrivate()
    {
        ...
        try
        {
            File file = ModLoader.getMinecraftServerInstance().func_71209_f("banned-mods.txt");
            if(!file.exists())
            {
                file.createNewFile();
            }
    ...
    {
        entityplayermp.field_71135_a.func_72567_b(packet230modloader);
    }
    ...
    private static void handleModCheckResponse(Packet230ModLoader packet230modloader, EntityPlayerMP entityplayermp)
    {
        ...
        log((new StringBuilder()).append(entityplayermp.field_71092_bJ).append(" joined with ").append(stringbuilder.toString()).toString());
        ...
        if(arraylist.size() != 0)
        {
            ...
            log((new StringBuilder()).append(entityplayermp.field_71092_bJ).append(" kicked for having ").append(stringbuilder1.toString()).toString());
            ...
            entityplayermp.field_71135_a.func_72565_c((new StringBuilder()).append("The following mods are banned on this server:").append(stringbuilder3.toString()).toString());
        } else
        if(arraylist1.size() != 0)
        {
            ...
            entityplayermp.field_71135_a.func_72565_c((new StringBuilder()).append("You are missing the following mods:").append(stringbuilder2.toString()).toString());
        }
    }
    ...
    {
        List list = ModLoader.getMinecraftServerInstance().field_71318_t.field_72404_b;
        for(int i = 0; i < list.size(); i++)
        {
            EntityPlayerMP entityplayermp = (EntityPlayerMP)list.get(i);
            entityplayermp.field_71135_a.func_72567_b(new Packet3Chat(s));
        }
    ...
    public static void sendChatToOps(String s)
    {
        ...
        {
            EntityPlayerMP entityplayermp = (EntityPlayerMP)list.get(i);
            if(ModLoader.getMinecraftServerInstance().field_71318_t.func_72353_e(entityplayermp.field_71092_bJ))
            {
                entityplayermp.field_71135_a.func_72567_b(new Packet3Chat(s));
            }
        }
    ...
    public static Packet getTileEntityPacket(BaseModMp basemodmp, int i, int j, int k, int l, int ai[], float af[], String as[])
    {
        ...
        packet230modloader.field_73287_r = true;
    ...
    {
        sendPacketToAll(tileentity.func_70319_e());
    }
