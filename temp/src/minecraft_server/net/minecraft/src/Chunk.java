    ...
    public Chunk(World p_i3772_1_, byte p_i3772_2_[], int p_i3772_3_, int p_i3772_4_)
    {
        ...
        for(int j = 0; j < 16; j++)
        {
            for(int k = 0; k < 16; k++)
            {
                for(int l = 0; l < i; l++)
                {
                    byte byte0 = p_i3772_2_[j << 11 | k << 7 | l];
                    if(byte0 == 0)
                    {
                        continue;
                    }
                    int i1 = l >> 4;
                    if(field_76652_q[i1] == null)
                    {
                        field_76652_q[i1] = new ExtendedBlockStorage(i1 << 4);
                    }
                    field_76652_q[i1].func_76655_a(j, l & 0xf, k, byte0);
                }
    ...
    public int func_76626_d(int p_76626_1_, int p_76626_2_)
    {
        ...
        if(j == -999)
        {
            ...
            for(j = -1; k > 0 && j == -1;)
            {
                ...
                Material material = l != 0 ? Block.field_71973_m[l].field_72018_cp : Material.field_76249_a;
