// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) braces deadcode fieldsfirst 

package net.minecraft.src;

import java.util.List;
import java.util.Random;

// Referenced classes of package net.minecraft.src:
//            Block, Material, World, EntityPlayer, 
//            InventoryPlayer, ItemStack, Item, PlayerCapabilities, 
//            EntityItem, EntityPlayerMP, AxisAlignedBB, Entity

public class BlockCauldron extends Block
{

    public BlockCauldron(int p_i3927_1_)
    {
        super(p_i3927_1_, Material.field_76243_f);
        field_72059_bZ = 154;
    }

    public int func_71858_a(int p_71858_1_, int p_71858_2_)
    {
        if(p_71858_1_ == 1)
        {
            return 138;
        }
        return p_71858_1_ != 0 ? 154 : 155;
    }

    public void func_71871_a(World p_71871_1_, int p_71871_2_, int p_71871_3_, int p_71871_4_, AxisAlignedBB p_71871_5_, List p_71871_6_, Entity p_71871_7_)
    {
        func_71905_a(0.0F, 0.0F, 0.0F, 1.0F, 0.3125F, 1.0F);
        super.func_71871_a(p_71871_1_, p_71871_2_, p_71871_3_, p_71871_4_, p_71871_5_, p_71871_6_, p_71871_7_);
        float f = 0.125F;
        func_71905_a(0.0F, 0.0F, 0.0F, f, 1.0F, 1.0F);
        super.func_71871_a(p_71871_1_, p_71871_2_, p_71871_3_, p_71871_4_, p_71871_5_, p_71871_6_, p_71871_7_);
        func_71905_a(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, f);
        super.func_71871_a(p_71871_1_, p_71871_2_, p_71871_3_, p_71871_4_, p_71871_5_, p_71871_6_, p_71871_7_);
        func_71905_a(1.0F - f, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F);
        super.func_71871_a(p_71871_1_, p_71871_2_, p_71871_3_, p_71871_4_, p_71871_5_, p_71871_6_, p_71871_7_);
        func_71905_a(0.0F, 0.0F, 1.0F - f, 1.0F, 1.0F, 1.0F);
        super.func_71871_a(p_71871_1_, p_71871_2_, p_71871_3_, p_71871_4_, p_71871_5_, p_71871_6_, p_71871_7_);
        func_71919_f();
    }

    public void func_71919_f()
    {
        func_71905_a(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F);
    }

    public boolean func_71926_d()
    {
        return false;
    }

    public int func_71857_b()
    {
        return 24;
    }

    public boolean func_71886_c()
    {
        return false;
    }

    public boolean func_71903_a(World p_71903_1_, int p_71903_2_, int p_71903_3_, int p_71903_4_, EntityPlayer p_71903_5_, int p_71903_6_, float p_71903_7_, 
            float p_71903_8_, float p_71903_9_)
    {
        if(p_71903_1_.field_72995_K)
        {
            return true;
        }
        ItemStack itemstack = p_71903_5_.field_71071_by.func_70448_g();
        if(itemstack == null)
        {
            return true;
        }
        int i = p_71903_1_.func_72805_g(p_71903_2_, p_71903_3_, p_71903_4_);
        if(itemstack.field_77993_c == Item.field_77786_ax.field_77779_bT)
        {
            if(i < 3)
            {
                if(!p_71903_5_.field_71075_bZ.field_75098_d)
                {
                    p_71903_5_.field_71071_by.func_70299_a(p_71903_5_.field_71071_by.field_70461_c, new ItemStack(Item.field_77788_aw));
                }
                p_71903_1_.func_72921_c(p_71903_2_, p_71903_3_, p_71903_4_, 3);
            }
            return true;
        }
        if(itemstack.field_77993_c == Item.field_77729_bt.field_77779_bT && i > 0)
        {
            ItemStack itemstack1 = new ItemStack(Item.field_77726_bs, 1, 0);
            if(!p_71903_5_.field_71071_by.func_70441_a(itemstack1))
            {
                p_71903_1_.func_72838_d(new EntityItem(p_71903_1_, (double)p_71903_2_ + 0.5D, (double)p_71903_3_ + 1.5D, (double)p_71903_4_ + 0.5D, itemstack1));
            } else
            if(p_71903_5_ instanceof EntityPlayerMP)
            {
                ((EntityPlayerMP)p_71903_5_).func_71120_a(p_71903_5_.field_71069_bz);
            }
            itemstack.field_77994_a--;
            if(itemstack.field_77994_a <= 0)
            {
                p_71903_5_.field_71071_by.func_70299_a(p_71903_5_.field_71071_by.field_70461_c, null);
            }
            p_71903_1_.func_72921_c(p_71903_2_, p_71903_3_, p_71903_4_, i - 1);
        }
        return true;
    }

    public void func_71892_f(World p_71892_1_, int p_71892_2_, int p_71892_3_, int p_71892_4_)
    {
        if(p_71892_1_.field_73012_v.nextInt(20) != 1)
        {
            return;
        }
        int i = p_71892_1_.func_72805_g(p_71892_2_, p_71892_3_, p_71892_4_);
        if(i < 3)
        {
            p_71892_1_.func_72921_c(p_71892_2_, p_71892_3_, p_71892_4_, i + 1);
        }
    }

    public int func_71885_a(int p_71885_1_, Random p_71885_2_, int p_71885_3_)
    {
        return Item.field_77721_bz.field_77779_bT;
    }

    public int func_71922_a(World p_71922_1_, int p_71922_2_, int p_71922_3_, int p_71922_4_)
    {
        return Item.field_77721_bz.field_77779_bT;
    }
}
