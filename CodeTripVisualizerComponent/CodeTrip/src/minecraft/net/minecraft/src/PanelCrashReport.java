...
public class PanelCrashReport extends Panel
{
    public PanelCrashReport(CrashReport par1CrashReport)
    {
        ...
        try
        {
            ...
            s2 = (new StringBuilder()).append(s2).append(par1CrashReport.getSections()).toString();
        ...
        else
        {
            ...
            s3 = (new StringBuilder()).append(s3).append("      ----------------------      \n").toString();
            s3 = (new StringBuilder()).append(s3).append("\n").toString();
            s3 = (new StringBuilder()).append(s3).append("Minecraft has stopped running because it encountered a problem; ").append(par1CrashReport.getDescription()).append("\n").toString();
            File file = par1CrashReport.getFile();

            if (file == null)
            {
                par1CrashReport.saveToFile(new File(new File(Minecraft.getMinecraftDir(), "crash-reports"), (new StringBuilder()).append("crash-").append((new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss")).format(new Date())).append("-client.txt").toString()));
                file = par1CrashReport.getFile();
            }
