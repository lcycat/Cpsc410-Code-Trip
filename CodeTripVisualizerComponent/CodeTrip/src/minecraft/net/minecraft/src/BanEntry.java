...
public class BanEntry
{
    ...
    public static BanEntry parse(String par0Str)
    {
        ...
        try
        {
            banentry.setBanStartDate(dateFormat.parse(as[i].trim()));
        }
        catch (ParseException parseexception)
        {
            loggerBanEntry.log(Level.WARNING, (new StringBuilder()).append("Could not read creation date format for ban entry '").append(banentry.getBannedUsername()).append("' (was: '").append(as[i]).append("')").toString(), parseexception);
        }
        ...
        {
            loggerBanEntry.log(Level.WARNING, (new StringBuilder()).append("Could not read expiry date format for ban entry '").append(banentry.getBannedUsername()).append("' (was: '").append(as[i]).append("')").toString(), parseexception1);
        }
